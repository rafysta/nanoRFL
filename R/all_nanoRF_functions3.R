...
roc.curve <-
  function(matrix,
    ...
    for (i in 1:length(steps)) {
      TPs[i] <-
        length(which(matrix[which(as.numeric(matrix[, 1]) >= steps[i]), 2] == true.value)) /
        trues
      FPs[i] <-
        length(which(matrix[which(as.numeric(matrix[, 1]) >= steps[i]), 2] == false.value)) /
        falses
...
get.trainings <-
  function(table,
    ...
    only.trainings <-
      table[sort(c(grep(true.value, table[, 1]), grep(false.value, table[, 1]))), ]

    #as we lost at least one class in the factor, we have to shrink the value''s levels just by redefining the factor.
    only.trainings[, class.col] <-
      as.factor(as.vector(only.trainings[, class.col]))
...
run.random.forest <-
  function(dataframe,
    ...
    the.forest <-
      randomForest(TF.factor ~ ., data = training.frame, ntrees = trees)
    ...
    df.pred <-
      predict(the.forest,
              all.unknowns[, (training.factor + 1):dim(all.unknowns)[2]],
              norm.votes = TRUE,
              type = "prob")
...
mcc <-
  function(roc.matrix,
    ...
    tp <-
      length(which(roc.matrix[which(roc.matrix[, 1] > threshold), 2] == true.values))

    #cat("---",tp, "---\n")
    ##truenegatives will be the cases below the threshold which are called F
    tn <-
      length(which(roc.matrix[which(roc.matrix[, 1] < threshold), 2] == false.values))
    #cat("---",tn, "---\n")
    ...
    if (pduct == 0) {
      ...
      determinator <-
        (log10(tp + fp) + log10(tp + fn) + log10(tn + fp) + log10(tn + fn)) / 2
...
rf.workflow <- function(dataframe,
                        col = "red",
                        max.sensitivity = TRUE) {
  ...
  roc.input.matrix <-
    dataframe.with.predictors[order(dataframe.with.predictors$T, decreasing =
                                      T), c("T", "training.factor")]


  roc.output.matrix <- roc.curve(roc.input.matrix, color = col)
  par(new = T)
  classif.color <-
    plot.predictor.quality(dataframe.with.predictors[, "T"], dataframe.with.predictors[, "training.factor"])
  ...
  if (max.sensitivity == TRUE) {
    cutoff <-
      as.numeric(names(mcc.vector)[max.cutoffs.group[length(max.cutoffs.group)]])
  }
  else{
    cutoff <-
      as.numeric(names(mcc.vector)[max.cutoffs.group[length(max.cutoffs.group)[1]]])
  }
  ...
  return(
    list(
      PREDICTORS.SCORES = dataframe.with.predictors,
      ROC.INPUT = roc.input.matrix,
      ROC.OUTPUT = roc.output.matrix,
      MCC = mcc.vector,
      MAX.MCC = max.mcc,
      CUTOFF = cutoff,
      AUC = auc,
...
optimize.mcc <-
  function(hill.climbing.training.factor,
           prot.groups.table,
           max.increases = 15,
           max.cycles = 50,
           mutate = TRUE) {
    ...
    if (length(table(hill.climbing.training.factor)) < 3) {
      ...
      cat(
        "WARNING:ONLY ONE CATEGORY IN TRAINING SET. RETURN VALUE IS",
        mcc.hill.climbing,
        "\n"
      )
      ...
      forest <-
        rf.workflow(cbind(hill.climbing.training.factor, log2(prot.groups.table[, sampled.experiments])))
      ###now we are storing the experiments used for this run


      mcc.hill.climbing[[1]] <-
        list(
          mcc = max(forest$MCC, na.rm = T),
          samples = sampled.experiments,
          scores = forest$PREDICTORS.SCORES[, dim(forest$PREDICTORS.SCORES)[2]],
          training.factor = forest$PREDICTORS.SCORES[, 1],
          roc.output = forest$ROC.OUTPUT
        )
      mcc.hill.climbing[[1]]["cutoff"] <-
        names(forest$MCC)[which(forest$MCC == mcc.hill.climbing[[1]]$mcc)][1]
      ...
      while (i < max.increases && flag < max.cycles) {
        ...
        if (mutate == TRUE) {
          sampled.experiments <-
            mutate(mcc.hill.climbing[[x]]$samples, all.experiments)
        }
        forest <-
          rf.workflow(cbind(hill.climbing.training.factor, log2(protein.groups[, sampled.experiments])))
        ...
        mcc.hill.climbing[[i]]["cutoff"] <-
          names(forest$MCC)[which(forest$MCC == mcc.hill.climbing[[i]]$mcc)][1]
        #cat("cutoff-------\n")
        ...
        if (mcc.hill.climbing[[i]]$mcc <= mcc.hill.climbing[[x]]$mcc) {
          ...
          cat("**Successfully climbed to a higher mcc**: ",
              mcc.hill.climbing[[i]]$mcc,
              "\n")
          i <- i + 1
        }
...
id.category.format.check <- function(matrix) {
  ...
  for (i in 1:dim(matrix)[1]) {
    ...
    if (d > 0) {
      #cat("found an or\n")
      new.lines <-
        rbind(new.lines, cbind(strsplit(matrix[i, 1][1], split = " or |;")[[1]], matrix[i, 2]))
...
convert.trf.elements <-
  function(training.factor, elements, convert.to = "F") {
...
oprafor.gts <-
  function(id.cat.matrix,
    ...
    cat("getting IDs from category ",
        positive,
        " to make the positive training set...\n")
    some.category.ids.positive <-
      get.category.ids(formatted.table, positive)
    cat("Identifying positive proteins in the protein table...")

    positive.prots <-
      retrieve.proteins.identified(id.list = some.category.ids.positive, prot.group.table =
                                     variable.table)
    ...
    cat("getting IDs from category ",
        negative,
        " to make the negative training set...\n")

    some.category.ids.negative <-
      get.category.ids(formatted.table, negative)
    cat("Identifying negative training proteins in the protein table...")
    negative.prots <-
      retrieve.proteins.identified(id.list = some.category.ids.negative, prot.group.table =
                                     variable.table)
    ...
    out <-
      optimize.mcc(
        new.training.factor,
        prot.groups.table = variable.table[, variable.indices],
        max.cycles = number.of.cycles ,
        mutate = set.mutate
      )
...
LOO.experiment <-
  function(rf.object,
    ...
    if (is.na(optimization) == FALSE) {
      optimization <-
        optimize.mcc(
          rf.object$training.factor,
          predictor.table[, rf.object$samples],
          max.increases = 15,
          max.cycles = 15
        )
    ...
    #cat("flag1\n")
    loo.summary <-
      matrix(ncol = 2, nrow = length(rf.object[length(rf.object)][[1]]$samples) +
               1)

    colnames(loo.summary) <- c("final.mcc", "improved")

    rownames(loo.summary) <-
      c(NA, rf.object[length(rf.object)][[1]]$samples)

    #cat("flag2\n")
    print(dim(loo.summary))

    loo.summary[1, ] <-
      c(mcc.to.improve, as.character(did.it.improve(mcc.to.improve, mcc.to.improve)))

    output <- list()
    #cat("flag22\n")
    output$BEST.PREDICTOR.SCORES <-
      optimization[length(optimization)][[1]]$scores
    #cat("flag222\n")
    output$BEST.ROC.OUTPUT <-
      optimization[length(optimization)][[1]]$roc.output
    #cat("flag2222\n")
    output$BEST.TRAINING.FACTOR <-
      optimization[length(optimization)][[1]]$training.factor
    #cat("flag3\n")
    ...
    for (i in 1:(dim(loo.summary)[1] - 1)) {
      #cat("flag4\n")
      new.experiment.set <-
        rf.object[length(rf.object)][[1]]$samples[-i]
      #cat("flag44\n")
      optimization <-
        optimize.mcc(
          rf.object[length(rf.object)][[1]]$training.factor,
          predictor.table[, new.experiment.set],
          max.cycles = cycles,
          mutate = F
        )
      #cat("flag444\n")
      loo.summary[i + 1, c(1, 2)] <-
        c(optimization[length(optimization)][[1]]$mcc,
          did.it.improve(
            as.numeric(mcc.to.improve),
            as.numeric(optimization[length(optimization)][[1]]$mcc)
          ))
      #cat("flag5\n")
...
plot.predictor.quality <- function(predictor, response, height = 0.05) {
  ...
  pred.colors <-
    sapply(as.vector(response[order(predictor, decreasing = T)]), class2color.TF, USE.NAMES =
             F)
  barplot(
    rep(height, length(predictor)),
    border = NA,
    col = pred.colors,
    yaxt = "n",
    ylim = c(0, 1)
...
class.score2color <-
  function(tf,
           score,
           cutoff,
           colors = c("light blue", "red", "orange"),
           nacolor = NA) {
...
plot.classified.proteins2 <-
  function(rf1,
           rf2,
           color.vector = class2color.2TFs(rf1$PREDICTORS.SCORES[, "training.factor"], rf2$PREDICTORS.SCORES[, "training.factor"], nacolor =
                                             NA),
           name.vector = NA,
           distance.from.border = .2,
           plot.character = class2pch.2TFs(rf1$PREDICTORS.SCORES[, "training.factor"], rf2$PREDICTORS.SCORES[, "training.factor"], nacolor =
                                             NA),
           char.size = .5,
           text.size = char.size,
           box = "o",
           ...
           id.vector = NA,
           border.center = F,
           significance.gradient = F) {
    if (border.center == TRUE) {
      classifier1 <-
        as.numeric(rf1$PREDICTORS.SCORES[, "T"]) - as.numeric(rf1$CUTOFF)
      classifier2 <-
        as.numeric(rf2$PREDICTORS.SCORES[, "T"]) - as.numeric(rf2$CUTOFF)
      cutoff1 <- 0
      cutoff2 <- 0
    } else{
    ...
    if (plot.names == T) {
      for (i in 1:length(classifier2)) {
            ...
            if (rf2$PREDICTORS.SCORES[i, "training.factor"] == 'T') {
              ...
              if (classifier2[i] > cutoff2 || classifier1[i] > cutoff1) {
                ...
                if (plot.ids == T && is.na(id.vector) == F) {
                  if (classifier2[i] > cutoff2 ||
                      classifier1[i] > cutoff1 ||
                      rf1$PREDICTORS.SCORES[i, "training.factor"] == 'T' ||
                      rf2$PREDICTORS.SCORES[i, "training.factor"] == 'T') {
                    text(
                      y = classifier2[i] - offset,
                      x = classifier1[i],
                      name.vector[i],
        ...
        if ((classifier2[i] > cutoff2 &&
             rf2$PREDICTORS.SCORES[i, "training.factor"] == '?') ||
            (classifier1[i] > cutoff1 &&
             rf1$PREDICTORS.SCORES[i, "training.factor"] == '?')) {
          points(
            y = classifier2[i] - offset,
            x = classifier1[i],
            cex = char.size ,
...
trapezium.auc <- function(roc.output.matrix, height = .001) {
  ...
  for (i in 2:nrow(roc.output.matrix))
    ...
    sum.vector[i - 1] <-
    ((roc.output.matrix[i, 2] + roc.output.matrix[i - 1, 2]) * height) / 2
...
generateRandomClassifiers <-
  function(numObservations, numClassifiers) {
...
reindex.experiment <-
  function(source.table,
           target.table,
           target.table.name.index,
           source.table.name.index,
           source.table.value.index) {
    out <- rep(NA, nrow(target.table))
    for (i in 1:nrow(target.table)) {
      result <-
        grep (target.table[i, target.table.name.index], source.table[, source.table.name.index])
      cat(result, "\n\n")
      if (length(result) == 0) {
        out[i] <- NA
      } else{
...
random.ISCoF.iterations <-
  function(dataset = 1,
           ...
           numEvents = 10000) {
    if (length(dataset == 1)) {
      toyDataset <- matrix(nrow = rows,
                           ncol = cols,
                           data = runif(rows * cols))
    ...
    for (i in 1:numEvents) {
      ...
      fake1.tf <-
        merge.vectors(positive = queryIndices, negative = referenceIndices)
      fake1.rf <-
        rf.workflow(cbind(fake1.tf, as.data.frame(toyDataset)))
...
random.ISCoF <-
  function(dataset = 1,
    ...
    centroids <-
      sample(1:rowz, querySize + refSize) ##These centroids will be both positive and negative training sets.
    query <- centroids[1:querySize]
    reference <- centroids[querySize + 1:refSize]
...
plotErrorBars <- function(matr) {
  errbar(
    x = 1:ncol(matr),
    y = colMeans(matr),
    yplus = colMeans(matr) + apply(matr, 1, sd),
    yminus = colMeans(matr) - apply(matr, 1, sd)
  )
